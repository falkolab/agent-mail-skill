...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --no-hooks)     shift ;;                     # accepted and ignored: never registers anyway
...
if [ "$REMOVE" = "1" ]; then
  ...
  for t in "${TARGETS[@]}"; do
    [ -d "$t" ] || continue
    "$HERE/amq-uninstall-project-hooks.sh" --dir "$t" 2>/dev/null | sed "s|^|  $(basename "$t"): |"
...
# -- 6. delivery hooks ---------------------------------------------------
# This script does not register hooks, and there is no flag that makes it. Registration
# belongs to amq-install-user-hooks.sh alone, so there is exactly one layer and nothing
# can stack. The opt-in that used to live here was justified by cloud sessions reading a
# committed settings.json — but a cloud session clones a repository whose mailbox and
# .amqrc are git-excluded, so it has no mailbox to read and the hook exits silently
# there anyway. The flag bought nothing and cost a second layer.
